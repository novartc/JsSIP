    ...
    Promise.resolve()
      ...
      .then((stream) =>
      {
        ...
        if (stream)
        {
          stream.getTracks().forEach((track) =>
          {
            if (track.kind === 'audio' && directionAudio === 'recvonly')
            {
              this._connection.addTransceiver('audio', { direction: 'recvonly', streams: [ stream ] });

              return;
            }
            if (track.kind === 'video' && directionVideo === 'recvonly')
            {
              this._connection.addTransceiver('video', { direction: 'recvonly', streams: [ stream ] });
